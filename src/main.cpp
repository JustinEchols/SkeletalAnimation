...
/*
 ...
 [] Mesh initialization
	...
	[X] Move animation data outside mesh struct
	...
	if(glfwInit())
	{
		...
		if(Window.Handle)
		{
			...
			if(glewInit() == GLEW_OK)
			{
...
#if 0
				mat4 T = Model.Meshes[0].Joints[0].Transform;

				v3 X = Mat4Column(T, 0).xyz;
				v3 Y = Mat4Column(T, 1).xyz;
				v3 Z = Mat4Column(T, 2).xyz;
				v3 P = Mat4Column(T, 3).xyz;

				f32 Sx = Length(X);
				f32 Sy = Length(Y);
				f32 Sz = Length(Z);

				mat4 R = Mat4(Sx * X, Sy * Y, Sz * Z);
				...
				quaternion Q = {};
				Q.w = 0.5f * sqrt(Max(0.0f, 1 + Trace));
				Q.x = 0.5f * sqrt(Max(0.0f, 1 + R00 - R11 - R22));
				Q.y = 0.5f * sqrt(Max(0.0f, 1 - R00 + R11 - R22));
				Q.z = 0.5f * sqrt(Max(0.0f, 1 - R00 - R11 + R22));
#endif
				...
				while(!glfwWindowShouldClose(Window.Handle))
				{
					...
					//
					// NOTE(Justin): This is a janky version of animation.
					//
